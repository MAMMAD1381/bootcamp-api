...
exports.forgotPassword = asyncHandler(async function (req, res, next) {
  ...
  let url = `<a href="${req.protocol}://${req.get(
    "host"
  )}/api/v1/resetpassword/${resetToken}">reset password</a>`;
  console.log(url);
    ...
    html: `<form method="post" action="some_page" class="inline">
        <center style="width: 100%; background-color: #0d0cb5; padding: 10px;">
            ...
            <button style="height: 50px; background-color: #0d0cb5; display: inline-block; border-radius: 5px; border: 1px solid #fff;" type="submit" name="submit_param" value="submit_value" class="link-button">
                <p><a style="text-decoration: none; padding: 5px 15px; display: inline-block; border-radius: 5px; background: transparent; color: #fff; href="${url}"> </a></p>
...
exports.resetPassword = asyncHandler(async function(req, res, next){
    if(!req.params.token) next(new errorMessage('pls provide a reset password token', 400))

    let token = req.params.token
    let hashedToken = crypto.createHash('sha256').update(token).digest('hex')+''

    let user = await User.findOne({resetPasswordToken: hashedToken})
    if (!user) next(new errorMessage('wrong reset password token', 401))
    
    if(Date.now > user.resetPasswordExpire) next(new errorMessage('reset password token expired', 403))

    if(!req.body.newPassword) next(new errorMessage('providing the new password is required', 400))
    ...
    res.status(200).send({success: true, data: 'the reset password was successful'})
})
